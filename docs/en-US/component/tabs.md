...
| left-type ^(2.14.6)     | type of tabs fixed on the left; inherits `type` when empty                                                                              | ^[enum]`'' \| 'card' \| 'border-card'`                                                           | ''         |
| right-type ^(2.14.6)    | type of tabs fixed on the right; inherits `type` when empty                                                                             | ^[enum]`'' \| 'card' \| 'border-card'`                                                           | ''         |
...
| closable        | whether Tab is closable                                                                                                                                                             | ^[boolean]                 | false   |
| fixed ^(2.14.6) | fixes the tab at the left or right edge; fixed tabs do not participate in scrolling                                                                                                 | ^[enum]`'left' \| 'right'` | —       |
